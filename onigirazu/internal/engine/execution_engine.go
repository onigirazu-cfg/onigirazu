...
type ExecutionEngine struct {
	...
	extraVars map[string]interface{}            // Extra vars from command line (highest priority)
...
func (e *ExecutionEngine) ExecutePlaybook(ctx context.Context, playbook *types.Playbook) (*types.PlaybookResult, error) {
	...
	// Load variables in priority order (lowest to highest):
	...
	// 3. Extra vars (--extra-vars) - highest priority
	// Apply extra vars after environment variables to ensure they override everything
	e.applyExtraVars()
...
// SetExtraVars sets extra variables (highest priority)
// These variables override all other variable sources
func (e *ExecutionEngine) SetExtraVars(vars map[string]interface{}) {
	e.mutex.Lock()
	defer e.mutex.Unlock()

	// Store extra vars separately to maintain priority
	for k, v := range vars {
		e.extraVars[k] = v
	}
	e.logger.Debug("Set %d extra variables", len(vars))
}

// LoadEnvironmentVariables loads variables from environment with ONIGIRAZU_VAR_ prefix
// Example: ONIGIRAZU_VAR_my_var=value becomes my_var=value in playbook variables
func (e *ExecutionEngine) LoadEnvironmentVariables() {
	e.mutex.Lock()
	defer e.mutex.Unlock()

	const prefix = "ONIGIRAZU_VAR_"
	count := 0
	...
	for _, env := range os.Environ() {
		...
		if strings.HasPrefix(key, prefix) {
			// Remove prefix and convert to lowercase for variable name
			varName := strings.TrimPrefix(key, prefix)
			varName = strings.ToLower(varName)

			e.variables[varName] = value
			count++
			e.logger.Debug("Loaded environment variable: %s=%s", varName, value)
		}
	}

	if count > 0 {
		e.logger.Debug("Loaded %d variables from environment", count)
	}
}

// applyExtraVars applies extra variables with highest priority
// This is called internally after loading environment variables
func (e *ExecutionEngine) applyExtraVars() {
	e.mutex.Lock()
	defer e.mutex.Unlock()

	if len(e.extraVars) == 0 {
		return
	}
	...
	e.logger.Debug("Applied %d extra variables with highest priority", len(e.extraVars))
}

// GetVariables returns a copy of current variables (for testing purposes)
func (e *ExecutionEngine) GetVariables() map[string]interface{} {
	e.mutex.RLock()
