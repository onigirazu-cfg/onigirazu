...
func (m *MockConfig) GetSSHDefaultUser() string {
	args := m.Called()
	return args.String(0)
}

func (m *MockConfig) GetSSHDefaultPort() int {
	args := m.Called()
	return args.Int(0)
}

func (m *MockConfig) GetSSHDefaultKeyFile() string {
	args := m.Called()
	return args.String(0)
}
...
// Test SetExtraVars
func TestSetExtraVars(t *testing.T) {
	engine, _, mockLogger, _, _, _ := createTestEngine()
	...
	engine.SetExtraVars(extraVars)

	// Extra vars are stored separately and applied during ExecutePlaybook
	// We need to manually apply them for testing
	engine.applyExtraVars()
...
// Test SetExtraVars with nil
func TestSetExtraVars_Nil(t *testing.T) {
	engine, _, mockLogger, _, _, _ := createTestEngine()

	mockLogger.On("Debug", mock.Anything, mock.Anything).Return()

	// Set some initial vars
	engine.SetExtraVars(map[string]interface{}{"test": "value"})
	engine.applyExtraVars()

	// Setting nil should not panic
	engine.SetExtraVars(nil)
...
// Test SetExtraVars overwrites existing vars
func TestSetExtraVars_Overwrite(t *testing.T) {
	engine, _, mockLogger, _, _, _ := createTestEngine()

	mockLogger.On("Debug", mock.Anything, mock.Anything).Return()

	// Set initial vars
	engine.SetExtraVars(map[string]interface{}{
		"version": "1.0.0",
		"env":     "dev",
	})
	engine.applyExtraVars()

	// Set extra vars that should overwrite
	extraVars := map[string]interface{}{
		"version": "2.0.0",
	}
	engine.SetExtraVars(extraVars)
	...
	// Version should be overwritten, env should remain
	vars := engine.GetVariables()
	assert.Equal(t, "2.0.0", vars["version"])
...
func TestLoadEnvironmentVariables(t *testing.T) {
	...
	t.Setenv("ONIGIRAZU_VAR_test_var", "test_value")
	t.Setenv("ONIGIRAZU_VAR_another_var", "another_value")
	t.Setenv("ONIGIRAZU_VAR_NUMBER", "123")
	t.Setenv("OTHER_VAR", "should_not_be_loaded")

	// Mock logger calls
	mockLogger.On("Debug", mock.Anything, mock.Anything).Return()

	engine.LoadEnvironmentVariables()

	// Verify only ONIGIRAZU_VAR_ prefixed variables are loaded
	vars := engine.GetVariables()
	assert.Equal(t, "test_value", vars["test_var"])
	assert.Equal(t, "another_value", vars["another_var"])
	assert.Equal(t, "123", vars["number"]) // Should be lowercase
	assert.Nil(t, vars["OTHER_VAR"])
	assert.Nil(t, vars["other_var"])
}

// Test LoadEnvironmentVariables with no env vars
func TestLoadEnvironmentVariables_NoVars(t *testing.T) {
	engine, _, mockLogger, _, _, _ := createTestEngine()

	// Mock logger calls
	mockLogger.On("Debug", mock.Anything, mock.Anything).Return()

	// Should not panic with no environment variables
	engine.LoadEnvironmentVariables()
...
// Test variable priority: playbook vars < env vars < extra vars
func TestVariablePriority(t *testing.T) {
	engine, mockConfig, mockLogger, mockInventoryMgr, mockModuleRegistry, mockTemplateEngine := createTestEngine()

	// Mock config calls
	mockConfig.On("GetDryRun").Return(false)
	mockConfig.On("GetCheckMode").Return(false)
	...
	t.Setenv("ONIGIRAZU_VAR_version", "2.0.0")
	...
	mockInventoryMgr.hosts = []types.Host{host}
	mockInventoryMgr.On("GetHosts", "all").Return(mockInventoryMgr.hosts, nil)

	// Mock template engine to capture rendered variables
	var capturedVars map[string]interface{}
	mockTemplateEngine.On("RenderTaskArgs", mock.Anything, mock.Anything, mock.Anything).
		Run(func(args mock.Arguments) {
			capturedVars = args.Get(2).(map[string]interface{})
		}).
		Return(map[string]interface{}{"msg": "test"}, nil)
	...
	mockModuleRegistry.On("ExecuteTask", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		Return(types.TaskResult{
			Success: true,
			Changed: false,
			Output:  map[string]interface{}{"msg": "ok"},
		}, nil)
				...
				Tasks: []types.Task{
					{
						Name:   "test task",
						Module: "debug",
						Args:   map[string]interface{}{"msg": "{{ version }}"},
					},
				},
			},
		},
	...
	// Set extra vars (highest priority)
	engine.SetExtraVars(map[string]interface{}{
		"version": "3.0.0",
	})

	ctx := context.Background()
	_, err := engine.ExecutePlaybook(ctx, playbook)

	assert.NoError(t, err)

	// Verify that extra vars have highest priority
	// The version in capturedVars should be "3.0.0" from extra vars
	assert.NotNil(t, capturedVars)
	assert.Equal(t, "3.0.0", capturedVars["version"])
}

// Test concurrent access to vars (thread safety)
func TestVariableConcurrency(t *testing.T) {
	...
	// Concurrent writes via SetExtraVars
	...
	// Concurrent reads via LoadEnvironmentVariables
	for i := 0; i < iterations; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
