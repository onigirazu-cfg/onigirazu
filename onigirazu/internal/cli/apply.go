...
Variables can be passed in multiple ways with the following priority (lowest to highest):
  1. Playbook vars section
  2. Environment variables (ONIGIRAZU_VAR_*)
  3. Command-line extra vars (--extra-vars)
...
Examples:
  ...
  onigirazu apply production.yml -e version=1.2.3 -e env=production
  ...
  onigirazu apply production.yml --verbose`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			inventoryManager.SetConfig(cfg)
			...
			// Set extra variables if provided (highest priority)
			if len(extraVars) > 0 {
				extraVarsInterface := make(map[string]interface{})
				for k, v := range extraVars {
					extraVarsInterface[k] = v
				}
				executionEngine.SetExtraVars(extraVarsInterface)
				log.Info("Applied %d extra variables from command line", len(extraVars))
			}
	...
	cmd.Flags().StringToStringVarP(&extraVars, "extra-vars", "e", map[string]string{}, "Extra variables (key=value, highest priority)")
